...
// 修改单元格样式：
diff, err = book.UpdateStyle(ctx, excelagent.UpdateStyleRequest{
	Sheet: "库存台账",
	Cell:  "D2",
	Style: excelagent.CellStyle{
		Bold:      true,
		FontColor: "FFFF0000",
		FillColor: "FFF2CC",
	},
...
// 写入计算公式：
diff, err = book.WriteFormula(ctx, excelagent.WriteFormulaRequest{
	Sheet:   "库存台账",
	Cell:    "E2",
	Formula: "SUM(D2:D10)",
})
...
### 指令序列链 (Command Pipeline)

若要进行原子性的多步复杂操作，可将指令以切片形式传给 `book.ExecuteSequence`：

```go
results, diff, err := book.ExecuteSequence(ctx, []excelagent.Command{
	{
		Op: "create_sheet",
		Target: excelagent.Target{Sheet: "新表"},
	},
	{
		Op: "update_cell",
		Target: excelagent.Target{Sheet: "新表", Cell: "A1"},
		Args: excelagent.UpdateCellArgs{Value: "标题"},
	},
	{
		Op: "update_style",
		Target: excelagent.Target{Sheet: "新表", Cell: "A1"},
		Args: excelagent.UpdateStyleArgs{Style: excelagent.CellStyle{Bold: true}},
	},
})
```
...
### 提示词分级字典 (Hierarchical Prompt Dictionary)

为避免单次向大模型输入过大的操作 Schema 和全部数据，`go-excel-agent` 支持把提示词拆解为三级结构（`LevelIntent` 任务规划路由、`LevelArgs` 参数生成提取、`LevelRepair` 出错自愈修复）。您可以使用以下方法一键渲染并获得精简后的 Prompt：

```go
// 渲染意图识别路由器 Prompt
request, err := book.RenderPrompt(excelagent.LevelIntent, excelagent.IntentPromptData{
	ExecutionHistory: "1. 成功读取了库存表列表",
	Query:            "帮我新建一张汇总表并把刚才的数据写入",
})
// request.SystemPrompt 与 request.Prompt 可直接作为 LLM 请求的 messages 传入
```
