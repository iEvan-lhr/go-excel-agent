...
`go-excel-agent` 是一个面向 Go 项目的 Excel 操作与 Agent 记忆内核。它提供稳定的 Excel API、强类型 DSL、保样式保存、Diff 追踪、工作簿结构索引、上下文胶囊和操作记忆能力。
...
engine/
  DSL 类型和确定性执行器，负责 find/update/batch_update/aggregate

memory/
  分层记忆、结构索引、上下文胶囊、操作账本、模型扩展接口
...
任何更新操作都应该返回 `Diff`。调用方可以根据 `ChangedCells` 判断是否需要保存，也可以把 Diff 写入操作账本。
